...
class RestClient implements Iterator, ArrayAccess {
    ...
    public function execute($url, $method='GET', $parameters=[], $headers=[]){
        ...
        // // // $client->url = $client->options['base_url'] . "&" . $parameters_string . '&moodlewsrestformat=json';
        // // $client->url = $client->options['base_url'] . "&" . $parameters_string;
        
        // var_dump($parameters_string); die;
        
        if ($parameters_string != "" && $method != "POST"){
            $client->url = $client->options['base_url'] . "&" . $parameters_string;
        }
